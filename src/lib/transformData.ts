      ...
      city.categories.forEach((category) => {
        ...
        category.temporal.forEach((year) => {
          year.data.forEach((month) => {
            const dateMonth = getMonthShortName(month.month, locale);

            const currentDate = new Date(parseInt(year.year), month.month - 1);
            ...
            if (!thisDate) {
              ...
              thisDate = temporalData.find((e) => e.date === currentDate);
            }
            ...
            const valueToAdd =
              ...
              (calculationMode == "absolute" ? 1 : month.activeUsers);

            const countryName =
              countryNames.find((e) => e.id === country.countryID)?.name ||
              country.countryID;

            thisDate![countryName] = (thisDate![countryName] || 0) + valueToAdd;
          });
        });
      }),
    );
    ...
    country.cities.forEach((city) => {
      for (const category of city.categories) {
        ...
        energyLabels.forEach((label) => {
          const findLabel = thisCategoryLabels.find((e) => e.label === label);
    ...
    country.cities.forEach((city) => {
      ...
      for (const category of city.categories) {
        ...
        if (summaryCategory) {
          ...
          summaryCategory.carbonEmissions += category.carbonEmissions ?? 0;
          summaryCategory.energyExpended += category.energyExpended ?? 0;
