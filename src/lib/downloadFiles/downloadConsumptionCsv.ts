...
export const getUserConsumptionsAsCsv = async (user: User) => {
    ...
    const consumptionsJson = consumptions.map((entry: Consumption) => {
        const res = {
            ...
            generatedByRecurringConsumption:
                entry.generatedByRecurringConsumptionId ? true : false,
            startDate: (() => {
                if (entry.category === "electricity") {
                    return (
                        formatTimestamp(entry.electricity?.startDate) || null
                    );
                } else if (entry.category === "heating") {
                    return formatTimestamp(entry.heating?.startDate) || null;
                } else if (entry.category === "transportation") {
                    return (
                        formatTimestamp(entry.transportation?.dateOfTravel) ||
                        null
                    );
                }
            })(),
            endDate: (() => {
                if (entry.category === "electricity") {
                    ...
                    return (
                        formatTimestamp(
                            entry.transportation?.dateOfTravelEnd,
                        ) || null
                    );
                }
            })(),
