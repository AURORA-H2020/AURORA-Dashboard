 ...
 * @return {ReactNode} - The rendered report component.
 */
...
const AutoReport = ({
  ...
  metaData?.sort((a, b) => {
    const aCountry =
      countriesMapping.find((e) => e.ID === a.countryID)?.name || a.countryID;
    const bCountry =
      countriesMapping.find((e) => e.ID === b.countryID)?.name || b.countryID;
  ...
  const report = (countries: string[]) => {
    ...
    const concatenatedCountries = (countries: string[]): string => {
      ...
      return countries.join(", ") + " " + t("common.and") + " " + lastCountry;
    };
    ...
    return (
      <Text className="text-md mb-4">
        ...
        {t("dashboard.autoReport.reportSnippets.totalConsumptions", {
          totalConsumptions: format.number(reportData.consumptions.total.count),
        })}{" "}
        {t("dashboard.autoReport.reportSnippets.consumptionBreakdown", {
          ...
          heatingCount: format.number(reportData.consumptions.heating.count),
        })}{" "}
        ...
        <Accordion type="multiple">
          {selectedCountryIds.map((countryID) => (
            <AccordionItem key={countryID} value={countryID}>
              <AccordionTrigger>
                {t(countriesMapping.find((e) => e.ID === countryID)?.name)}
              </AccordionTrigger>
              <AccordionContent>{report([countryID])}</AccordionContent>
            </AccordionItem>
          ))}
        </Accordion>
      )}
